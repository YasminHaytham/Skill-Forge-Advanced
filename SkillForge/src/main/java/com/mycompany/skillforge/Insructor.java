...
public class Insructor extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        ViewApprovalStatusBtn.setText("Approval Status");
        ViewApprovalStatusBtn.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ViewApprovalStatusBtnActionPerformed(evt);
            }
        });
    ...
    if (selectedRow >= 0) {
        ...
        Map<String, Object> analytics = instructor.getCourseAnalytics(selectedCourse.getCourseId());
        
        // Create and show insights frame
        InstructorInsights insightsFrame = new InstructorInsights(analytics, selectedCourse.getTitle());
    ...
    private void ViewApprovalStatusBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ViewApprovalStatusBtnActionPerformed
     int selectedRow = CourseTable.getSelectedRow();
    if (selectedRow >= 0) {
        Course selectedCourse = createdCourses.get(selectedRow);
        String status = instructor.getCourseApprovalStatus(selectedCourse.getCourseId());
        
        String message;
        switch (status) {
            case "PENDING":
                message = "Course is pending admin approval. Students cannot enroll yet.";
                break;
            case "APPROVED":
                message = "Course is approved and available for student enrollment.";
                break;
            case "REJECTED":
                message = "Course was rejected. Please contact admin for details.";
                break;
            default:
                message = "Approval status: " + status;
        }
        
        JOptionPane.showMessageDialog(this, message, "Course Approval Status", JOptionPane.INFORMATION_MESSAGE);
    } else {
        JOptionPane.showMessageDialog(this, "Please select a course first.");
    }
    }//GEN-LAST:event_ViewApprovalStatusBtnActionPerformed
    ...
    if (courseRow >= 0 && lessonRow >= 0) {
        Course selectedCourse = createdCourses.get(courseRow);
        Lesson selectedLesson = selectedCourse.getLessons().get(lessonRow);
        
        // OLD: new CreateQuiz().setVisible(true);
        // NEW: Pass the lesson to constructor
        CreateQuiz createQuizFrame = new CreateQuiz(selectedLesson);
    ...
    private javax.swing.JButton ViewApprovalStatusBtn;
